...
4. **Get a case-specific policy brief** — call `check_policy` with the customer's request and the verified order status, total, and prior-refund percentage. Ask for cancellation eligibility, ordering, the current net refund percentage, authority, and any escalation requirement. Do not copy a percentage from this procedure.
...
6. **Cancel** — `cancel_order(order_id, reason)` with a specific reason (audit-logged). On success you'll get a cancel ref. The system holds this call and asks the customer to confirm it before it runs: your turn ends on that question and picks up again from their answer. Call it when you're ready to cancel — don't ask for the same confirmation in your own words first, and don't describe the cancellation as done until the call comes back. If it returns `confirmation_denied`, nothing was changed: say so and answer whatever the customer said instead, without retrying.
7. **Refund the net** — `issue_refund(order_id, refund_percentage=net_pct, reason_code="cancellation", reason="cancel_net_of_prior")`. The server multiplies by `total_usd` and logs both pct and dollar amount. If the call returns a `policy_violation` 403 (over cap), DO NOT split — escalate the full case.
...
- ❌ Retrying `cancel_order` after a `confirmation_denied` — the customer said no; asking again by another route is not how you get a yes.
...
- `issue_refund(order_id, refund_percentage, reason_code, reason)` — the refund write. See `handle-refund` for the broader refund flow.
